...
$mode = isset($_GET['action'])&&is_number($_GET['action']) ? $_GET['action'] : 1;
...
$objTPL->set_filenames(array(
	'header' => 'core/installer/header.tpl',
	'footer' => 'core/installer/footer.tpl',
));
if(is_file(cmsROOT.'core/installer/step_'.$mode.'.tpl'))
    $objTPL->set_filenames(array(
    	'content' => 'core/installer/step_'.$mode.'.tpl'
    ));
...
switch($mode){
    ...
    default:
    case 1:
        $objTPL->assign_var('SUBMIT', $objForm->button('Next', 'submit', array('extra'=>' onclick="window.location=\'?action=2\'"')));
    ...
    case 2:
        ...
        if(isset($_GET['try'])){
    		...
    		$vars = array(
    			'Database Server'				      => $objForm->inputbox('input', 'localhost', 'db_server'),
    			'Database Username'				      => $objForm->inputbox('input', '', 'db_uname'),
    			'Database Password'				      => $objForm->inputbox('password', '', 'db_passwd'),
    			'Database Name'				          => $objForm->inputbox('input', '', 'db_db'),
    			'Database Prefix'				      => $objForm->inputbox('input', 'cscms_', 'db_prefix'),
    		);
            $objTPL->assign_vars(array(	
                'FORM_START'    =>  $objForm->start('databse', 'POST', '?action=2&try'),
                'FORM_END'      =>  $objForm->finish(),
                'SUBMIT'        =>  $objForm->button('Next', 'submit'),
            ));
    ...
    case 3:
		...
		$vars = array(
			'Administrator Username'			  => $objForm->inputbox('input', (isset($_POST['adm_username']) ? 
                                                                                    $_POST['adm_username'] : 'admin'), 'adm_username'),
			'Administrator Password'			  => $objForm->inputbox('password', (isset($_POST['adm_passwd']) ? 
                                                                                    $_POST['adm_passwd'] : ''), 'adm_passwd'),
			'Verify Password'			          => $objForm->inputbox('password', (isset($_POST['adm_v_passwd']) ? 
                                                                                    $_POST['adm_v_passwd'] : ''), 'adm_v_passwd'),
			'Administrator Email'			      => $objForm->inputbox('input', (isset($_POST['adm_email']) ? 
                                                                                    $_POST['adm_email'] : ''), 'adm_email'),
		);
        
        $objTPL->assign_vars(array(	
            'FORM_START'    =>  $objForm->start('databse', 'POST', '?action=3&try', 'return confirm(\'Only continue if you are happy with the details you provided.\')'),
            'FORM_END'      =>  $objForm->finish(),
            'SUBMIT'        =>  $objForm->button('Next', 'submit'),
        ));
    ...
    case 4:
		...
		$vars = array(
			'Site Title'				    => $objForm->inputbox('input', $title, 'title'),
            'Site Slogan'                   => $objForm->inputbox('input', $slogan, 'slogan'),
            'Site Description'              => $objForm->textarea($description, 'description'),
            'Site Keywords'                 => $objForm->textarea($keywords, 'keywords'),
            'Default Time Format'           => $objForm->inputbox('input', ($time==NULL ? 'jS F h:ia' : $time), 'time'),
            '&nbsp;'                        => '&nbsp;',
            'reCaptcha Public Key'          => $objForm->inputbox('input', $captcha_pub, 'captcha_pub'),
            'reCaptcha Private Key'         => $objForm->inputbox('input', $captcha_priv, 'captcha_priv'),
		);
        $objTPL->assign_vars(array(	
            'FORM_START'    => $objForm->start('siteConfig', 'POST', '?action=4&try'),
            'FORM_END'      => $objForm->finish(),
            'SUBMIT'        => $objForm->button('Next', 'submit'),
        ));
...
?>';
            ...
            $objTPL->assign_var('SUBMIT', $objForm->button('Next', 'submit', array('extra'=>' onclick="window.location=\'?action=6\'"')));
    ...
    //--SQL Setup
    //
        $objSQL     = new SQL($config['db'], true);
        //check and see whether we can connect to the db
        if(!$objSQL->connect((LOCALHOST ? true : false), true)){
        ...
        if(is_readable('sql.php')){
            ...
            if(is_empty($content)){
                ...
                $objTPL->assign_var('SUBMIT', $objForm->button('Finish', 'submit', array('extra' => ' onclick="window.location=\'/'.root().'index.php\'"')));
            }
...
$objTPL->pparse('header');
if(is_file(cmsROOT.'core/installer/step_'.$mode.'.tpl'))
    $objTPL->pparse('content');

$objTPL->pparse('footer');
?>